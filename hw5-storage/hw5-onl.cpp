...
// ArgumentManager am(argc, argv);
// ifstream file_in(am.get("input"));
// ifstream file_com(am.get("command"));
// ofstream file_out(am.get("output"));

ifstream file_in("input2.txt");
ifstream file_com("command2.txt");
...
file_com>>s_com;
    ...
    while(!file_com.eof()){
        file_com>>s_com;
        if(isdigit(s_com[0])){// fix this case
            s_level=s_com;
            cout<<s_level<<endl;
